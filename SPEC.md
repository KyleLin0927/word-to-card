...
2. **`notify.py`**: 
   ...
   - 提供 **`notify_success`**：在「實際新增 Anki 筆數 > 0」時，除通知中心外另觸發不依賴橫幅顯示的備援（預設短音效），以改善多螢幕／Focus 下看不到通知的情況。
3. **`queue_manager.py`**: 
...
- **例句目標詞標示（粗體＋底線）**：
  ...
  - **是否建議**：建議由模型在 `example_sentence` 內使用**固定、罕見於英文正文**的括號標記目標詞，再由程式轉成**粗體、底線**（字色與正文一致，主題預設）；**不建議**模型直接輸出任意 HTML（安全、樣式一致、易驗證）。
  ...
  - **產卡行為**：程式將 `⟦…⟧` 轉為**粗體、底線**、**字色繼承正文**（`color:inherit`）顯示（其餘字元 HTML escape）；**無**括號的舊例句仍整句斜體顯示，行為與過去相同。
...
## 需求：成功通知可靠性（多螢幕／通知中心）
### 背景
- macOS **通知橫幅**可能因 **專注模式（Focus）**、通知權限、或 **主顯示器／外接螢幕** 配置而不出現；`osascript display notification` 仍可能回傳成功，造成「有時有跳、有時沒跳」的感受。

### 行為
- 當流程確認 **Anki 實際新增筆數 > 0** 時，除呼叫既有 `display notification` 外，須另有 **不依賴通知中心是否顯示橫幅** 的備援回饋。
- **預設備援**：以系統 **`afplay`** 播放極短音效（非阻塞），確保使用者至少能聽到「已完成新增」。
- **可設定**：透過環境變數關閉或自訂音效檔路徑（見 `config.py` / `.env.example`）。

### 驗收條件
- **AC1**：`added > 0` 時，預設會觸發成功備援（音效）；與通知橫幅是否出現無關。
- **AC2**：關閉成功音效後，行為退回僅通知中心（與既有 `notify` 相同），不得影響寫入 Anki 與 log。
